...
class DeployBot(Resource):
    ...
    def post(self, bot_id):
        ...
        try:
            ...
            p1=Popen('rasa actions -p 8080',stdout=subprocess.PIPE,stderr=subprocess.PIPE,cwd=dirName1)
            p1.communicate()           
            p2=Popen('rasa run --credentials credentials.yml --endpoints ../pcf_instance_files/Dev/endpoints_dev.yml -p 5010 --enable-api --cors "*"',stdout=subprocess.PIPE,stderr=subprocess.PIPE,cwd=dirName2)
            p2.communicate()
...
#Infrastructure
#Deafault files while creating bot
#DB - Mongo or PostGre 
#Message Queues - Redis - Specific to bot training
#Intent handling along with entities
#Platform Independent & dependencies
#Upload the models from local 
#Use something specific to Rasa to validate the nlu 
